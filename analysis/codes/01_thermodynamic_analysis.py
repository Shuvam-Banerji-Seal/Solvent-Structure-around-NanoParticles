...
class ThermodynamicAnalyzer:
    ...
    def plot_temperature_evolution(self):
        ...
        for eps in self.epsilon_values:
            if eps in self.data:
                df = self.data[eps]
                ax1.plot(df['Time_ns'], df['Temp'], label=f'ε={eps}', alpha=0.7, linewidth=0.5)
        
        ax1.axhline(TARGET_TEMP, color='red', linestyle='--', linewidth=2, label='Target (300 K)')
        ax1.set_xlabel('Time (ns)')
        ax1.set_ylabel('Temperature (K)')
        ax1.set_title('Temperature Evolution During Production Run')
        ax1.legend(loc='best', ncol=2)
        ax1.grid(True, alpha=0.3)
        
        # Plot 2: Mean ± std for each epsilon
        ax2 = axes[1]
        ...
        ax2.axhline(TARGET_TEMP, color='red', linestyle='--', linewidth=2, label='Target')
        ax2.set_xticks(x_pos)
        ax2.set_xticklabels([f'{eps:.2f}' for eps in valid_eps])
        ax2.set_xlabel('Epsilon (kcal/mol)')
        ax2.set_ylabel('Mean Temperature (K)')
        ax2.set_title('Average Temperature by Epsilon Value')
        ax2.legend()
    ...
    def plot_pressure_evolution(self):
        ...
        for eps in self.epsilon_values:
            if eps in self.data:
                df = self.data[eps]
                ax1.plot(df['Time_ns'], df['Press'], label=f'ε={eps}', alpha=0.6, linewidth=0.5)
        
        ax1.axhline(TARGET_PRESS, color='red', linestyle='--', linewidth=2, label='Target (1 atm)')
        ax1.set_xlabel('Time (ns)')
        ax1.set_ylabel('Pressure (atm)')
        ax1.set_title('Pressure Evolution During Production Run')
        ax1.legend(loc='best', ncol=3)
        ax1.grid(True, alpha=0.3)
        
        # Plot 2: Pressure distribution (histogram)
        ax2 = fig.add_subplot(gs[1, 0])
        for eps in self.epsilon_values:
            if eps in self.data:
                df = self.data[eps]
                ax2.hist(df['Press'], bins=50, alpha=0.5, label=f'ε={eps}', density=True)
        
        ax2.axvline(TARGET_PRESS, color='red', linestyle='--', linewidth=2, label='Target')
        ...
        # Plot 3: Mean pressure by epsilon
        ...
        ax3.errorbar(x_pos, means, yerr=stds, fmt='o-', capsize=5, linewidth=2, markersize=8)
        ax3.axhline(TARGET_PRESS, color='red', linestyle='--', linewidth=2, label='Target')
        ax3.set_xticks(x_pos)
        ax3.set_xticklabels([f'{eps:.2f}' for eps in valid_eps])
        ax3.set_xlabel('Epsilon (kcal/mol)')
        ax3.set_ylabel('Mean Pressure (atm)')
        ax3.set_title('Average Pressure vs Epsilon')
        ax3.legend()
    ...
    def plot_density_analysis(self):
        ...
        # Plot 1: Density time series
        ax1 = axes[0, 0]
        for eps in self.epsilon_values:
            if eps in self.data:
                df = self.data[eps]
                ax1.plot(df['Time_ns'], df['Dens'], label=f'ε={eps}', alpha=0.7, linewidth=0.8)
        
        ax1.axhline(1.0, color='red', linestyle='--', linewidth=2, label='Water (1.0 g/cm³)')
        ax1.set_xlabel('Time (ns)')
        ...
        # Plot 2: Mean density vs epsilon
        ax2 = axes[0, 1]
        valid_eps = self.stats_df['Epsilon'].values
        x_pos = np.arange(len(valid_eps))
        ...
        ax2.errorbar(x_pos, means, yerr=stds, fmt='o-', capsize=5, linewidth=2, markersize=8,
                    color='darkblue')
        ax2.axhline(1.0, color='red', linestyle='--', linewidth=2, label='Pure water')
        ax2.set_xticks(x_pos)
        ax2.set_xticklabels([f'{eps:.2f}' for eps in valid_eps])
        ax2.set_xlabel('Epsilon (kcal/mol)')
        ax2.set_ylabel('Density (g/cm³)')
        ax2.set_title('Mean Density vs C-O Interaction Strength')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        
        # Plot 3: Volume evolution
        ax3 = axes[1, 0]
        for eps in self.epsilon_values:
            if eps in self.data:
                df = self.data[eps]
                ax3.plot(df['Time_ns'], df['Vol'], label=f'ε={eps}', alpha=0.7, linewidth=0.8)
        
        ax3.set_xlabel('Time (ns)')
        ax3.set_ylabel('Volume (Å³)')
        ax3.set_title('Box Volume Evolution')
        ...
        for eps in self.epsilon_values:
            if eps in self.data:
                df = self.data[eps]
                ax4.hist(df['Dens'], bins=30, alpha=0.5, label=f'ε={eps}', density=True)
        
        ax4.axvline(1.0, color='red', linestyle='--', linewidth=2, label='Pure water')
    ...
    def plot_energy_analysis(self):
        ...
        # Plot 1: PE time series
        ax1 = axes[0, 0]
        for eps in self.epsilon_values:
            if eps in self.data:
                df = self.data[eps]
                ax1.plot(df['Time_ns'], df['PE'], label=f'ε={eps}', alpha=0.7, linewidth=0.6)
        ...
        ax2.set_xticklabels([f'{eps:.2f}' for eps in valid_eps])
        ax2.set_xlabel('Epsilon (kcal/mol)')
        ax2.set_ylabel('Mean PE (kcal/mol)')
        ax2.set_title('Average Potential Energy vs C-O Interaction')
        ax2.grid(True, alpha=0.3)
        
        # Plot 3: PE distribution
        ax3 = axes[1, 0]
        for eps in self.epsilon_values:
            if eps in self.data:
                df = self.data[eps]
                ax3.hist(df['PE'], bins=40, alpha=0.5, label=f'ε={eps}', density=True)
        
        ax3.set_xlabel('Potential Energy (kcal/mol)')
        ax3.set_ylabel('Probability Density')
        ax3.set_title('PE Distribution')
        ax3.legend(loc='best')
        ax3.grid(True, alpha=0.3)
        
        # Plot 4: Energy fluctuations vs epsilon
        ax4 = axes[1, 1]
        valid_eps = self.stats_df['Epsilon'].values
        fluctuations = self.stats_df['PE_std'].values
        
        x_pos = np.arange(len(valid_eps))
        ax4.plot(x_pos, fluctuations, 'o-', linewidth=2, markersize=8, color='darkred')
        ax4.set_xticks(x_pos)
        ax4.set_xticklabels([f'{eps:.2f}' for eps in valid_eps])
        ax4.set_xlabel('Epsilon (kcal/mol)')
        ax4.set_ylabel('PE Standard Deviation (kcal/mol)')
        ax4.set_title('Energy Fluctuations vs C-O Interaction Strength')
        ax4.grid(True, alpha=0.3)

...
def visualize_improved_predictions():
    ...
    generated_dir = base_dir / "ml_integration/advanced/generated_improved"
    actual_base = base_dir / "solvent_effects"
    output_dir = base_dir / "ml_integration/advanced/logs_improved/plots"
        ...
        for i, pair in enumerate(pairs):
            ...
            if gen_file.exists() and act_file.exists():
                ...
                if len(rdf_gen) > 0 and len(rdf_act) > 0:
                    ...
                    ax.plot(rdf_gen[:, 0], rdf_gen[:, 1], 'r--', linewidth=2, label='Predicted (Improved)')
            ...
            if df_gen is not None and df_act is not None:
                ...
                for i, col in enumerate(cols):
                    ...
                    if col in df_gen.columns and col in df_act.columns:
                        ...
                        sns.kdeplot(df_gen[col], ax=ax, color='r', fill=True, alpha=0.3, label='Predicted')

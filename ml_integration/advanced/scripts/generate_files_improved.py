#!/usr/bin/env python3
"""
Generate MD files using the IMPROVED trained model.
===================================================

Modified to use ImprovedMDGenerativeModel.
...
class MDFileGeneratorImproved:
    def __init__(self, model_path: str, stats_path: str = "../logs_improved/normalization_stats.json", 
                 device: str = 'cuda'):
        self.device = device
        ...
        self.model = ImprovedMDGenerativeModel(
            ...
            thermo_seq_len=1000,
            rdf_bins=200,
            use_vae=False # Assuming VAE was disabled during training as per default
        ).to(device)
        ...
        with open(output_path, 'w') as f:
            for frame in range(n_frames):
                ...
                for i in range(self.n_atoms):
                    atom_type = 1 if i < self.n_c60 * 60 else 2 # C60 atoms are type 1, solvent type 2 (approx)
                    # Actually we don't have type info in latent space, assuming standard order
                    # C60 (180 mols * 60 atoms) = 10800 atoms? No, n_atoms is 5541.
                    # Let's just use type 1 for all for visualization or try to infer.
                    # For now, just writing coordinates is enough for OVITO if we guess types.
                    # Let's assume standard 1-based indexing
    ...
    def generate_thermo_file(self, epsilon: float, output_dir: Path):
        ...
        # Write file
        # Format: TimeStep Temp Press PotEng KinEng Volume Density
        # We need to estimate KE and Vol since we don't predict them directly/accurately yet
        # Or we can just write what we have. The comparison script expects specific columns.
        # compare_generated.py: names=['TimeStep', 'Temp', 'Press', 'PE', 'KE', 'Vol', 'Dens']
...
def main():
    parser = argparse.ArgumentParser(description="Generate MD files from IMPROVED model")
    ...
    print(f"\n============================================================")
    print(f"Generating files for epsilon = {args.epsilon:.2f} (IMPROVED MODEL)")
    print(f"Output directory: {output_dir}")
    print(f"============================================================\n")
    
    try:
        generator = MDFileGeneratorImproved(args.model, device=args.device)

...
def run_batch_comparison_improved():
    epsilons = [0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1.00, 1.05, 1.10]
    generated_base_dir = Path("../generated_improved")
    output_base_dir = Path("../logs_improved/comparison")
    output_base_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"🚀 Starting IMPROVED batch comparison analysis for {len(epsilons)} epsilons...")
    ...
    for eps in epsilons:
        ...
        try:
            ...
            # Compare RDFs - pass directories, not file paths
            rdf_metrics = compare_rdf(gen_dir, actual_dir)
            
            # Aggregate results
            res = {
                'epsilon': eps,
                'thermo_mse': {k: v['MSE'] for k, v in thermo_metrics.items()},
                'rdf_mse': {k: v.get('MSE', np.nan) for k, v in rdf_metrics.items() if v},
                'rdf_corr': {k: v.get('Corr', np.nan) for k, v in rdf_metrics.items() if v}
            }
            results.append(res)
...
def plot_summary(df, output_dir):
    ...
    plt.xlabel('Epsilon')
    plt.ylabel('RDF MSE')
    plt.title('RDF Prediction Error vs Epsilon (IMPROVED MODEL)')
    plt.legend()
    ...
    plt.title('Thermodynamic Property Error vs Epsilon (IMPROVED MODEL)')
    plt.legend()
    plt.grid(True, alpha=0.3)

#!/usr/bin/env python3
"""
Visualize VAE training progress and latent space.
"""
...
def plot_training_metrics(log_dir):
    history_file = log_dir / 'training_history_vae.json'
    ...
    plt.title('VAE Training Dynamics: Loss Curve')
    plt.legend()
    ...
    plt.title('VAE Model Uncertainty (Perplexity)')
    plt.grid(True, alpha=0.3)
    plt.savefig(log_dir / 'perplexity.png', dpi=300)
    plt.close()

def plot_latent_space(log_dir):
    csv_file = log_dir / 'latent_vectors_vae.csv'
...
def main():
    log_dir = Path('../logs_vae')
    plot_training_metrics(log_dir)
    plot_latent_space(log_dir)
    print("✅ Visualizations generated in ../logs_vae/")

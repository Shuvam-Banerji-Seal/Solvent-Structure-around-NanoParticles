...
Enables Variational Autoencoder (VAE) mode to enforce
a smooth Gaussian prior on the latent space.

This helps calibrate the magnitude of predicted properties
...
def visualize_latent_space(model: nn.Module, dataloader: DataLoader,
    ...
    ax.set_title(f'Latent Space (Epoch {epoch}) - VAE ENABLED', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(save_dir / f'latent_vae_epoch_{epoch:04d}.png', dpi=150)
    plt.close()
    
    # Save to CSV for detailed analysis (response function, AUC, etc.)
    csv_file = save_dir.parent / 'latent_vectors_vae.csv'
...
def train_improved(n_epochs: int = 1000,
    ...
    for epoch in epoch_pbar:
        ...
        # TRAIN
        model.train()
        train_losses = {k: 0.0 for k in ['total', 'trajectory', 'thermodynamics', 'rdf', 'smoothness', 'physics', 'vae']}
        ...
        # VALIDATE
        model.eval()
        val_losses = {k: 0.0 for k in ['total', 'trajectory', 'thermodynamics', 'rdf', 'smoothness', 'physics', 'vae']}
        ...
        if use_vae:
            tqdm.write(f"  [VAE: {train_losses['vae']:.4f}]")
        ...
        if improved:
            torch.save(checkpoint, checkpoint_dir / 'best_model_vae.pt')
            tqdm.write(f"  ✅ Best model saved")
        
        if epoch % 25 == 0:
            torch.save(checkpoint, checkpoint_dir / f'checkpoint_vae_epoch_{epoch:04d}.pt')
    ...
    # Final save
    torch.save(checkpoint, checkpoint_dir / 'final_model_vae.pt')
    ...
    with open(log_dir / 'training_history_vae.json', 'w') as f:
...
if __name__ == "__main__":
    ...
    # Print header
    print("\n" + "="*70)
    print("  VAE MD GENERATIVE MODEL TRAINING".center(70))
    print("="*70 + "\n")
    ...
    train_improved(
        ...
        checkpoint_dir='../checkpoints_vae',  # Separate dir
        log_dir='../logs_vae',  # Separate dir
        patience=1000
    )

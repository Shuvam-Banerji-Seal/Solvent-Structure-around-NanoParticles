...
# LAMMPS installation paths
LAMMPS_INSTALL_DIR="/home/iiserk/sbs/solvent/lammps-install"
LAMMPS_BIN="${LAMMPS_INSTALL_DIR}/bin/lmp_mpi"
LAMMPS_LIB64="${LAMMPS_INSTALL_DIR}/lib64"

# Set up library paths for LAMMPS
export LD_LIBRARY_PATH="${LAMMPS_LIB64}:${LD_LIBRARY_PATH}"
...
EPSILON_VALUES=(0.55 0.60 0.65 0.70 0.75 0.80 0.85 0.90 0.95 1.0 1.05 1.10)
RANDOM_SEED=42

# Base directory for all results (current working directory = 6ns_sim_v2)
RESULTS_BASE="$(pwd)"

# GPU and CPU resource allocation per simulation
export CUDA_VISIBLE_DEVICES=0  # Use GPU 0 (RTX 6000)
CORES_PER_JOB=5               # 10 cores per epsilon job (64 cores / 6 jobs = ~10.67)
OMP_THREADS_PER_JOB=5         # 10 OpenMP threads per epsilon
MAX_PARALLEL_JOBS=12            # Run up to 6 simulations simultaneously
...
echo "  Library Path:         ${LAMMPS_LIB64}"
...
echo "  CPU cores per job:    $CORES_PER_JOB cores per job (64 cores / 6 jobs)"
echo "  OMP threads per job:  $OMP_THREADS_PER_JOB"
echo "  Max parallel jobs:    $MAX_PARALLEL_JOBS"
echo "  Random seed:          $RANDOM_SEED"
echo ""
echo "Verifying LAMMPS installation..."
if [ ! -f "${LAMMPS_BIN}" ]; then
    echo "ERROR: LAMMPS binary not found at ${LAMMPS_BIN}"
    exit 1
fi
...
run_epsilon() {
    ...
    # Create isolated working directory in results base
    local work_dir="${RESULTS_BASE}/epsilon_${epsilon}"
    mkdir -p "$work_dir"
    cd "$work_dir"
    
    # Copy necessary files from results base
    cp "${RESULTS_BASE}/large_C60_solvated.data" .
    cp "${RESULTS_BASE}/H2O_TIP4P2005_fixed.mol" .
    ...
    local start_core=$(((job_id - 1) * CORES_PER_JOB))
    local end_core=$((start_core + CORES_PER_JOB - 1))
    
    echo "[$(date +%H:%M:%S)] Starting epsilon=$epsilon (Job $job_id, Cores $start_core-$end_core)"

    pwd
    
    # Run LAMMPS with VERSION 2 script (fully optimized)
    # CRITICAL: Using 2_equilibate_version_2.lmp (PPPM locked + GPU optimized)
    taskset -c $start_core-$end_core "${LAMMPS_BIN}" \
        -pk gpu 1 neigh yes newton off binsize 2.8 split 1.0 \
        -sf gpu \
        -var EPSILON_CO $epsilon \
        -var RANDOM_SEED $seed \
        -in "${RESULTS_BASE}/2_equilibrium_version_2_w_minimization.lmp" \
        -log equilibration.log \
        > equil_run.out 2>&1
    
    local exit_code=$?
    
    # Check completion and density validation - wait for log file first
    if [ $exit_code -eq 0 ] && [ -f "equilibration.log" ]; then
        if grep -q "Density validation PASSED" equilibration.log 2>/dev/null; then
